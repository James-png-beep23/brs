defmodule BrsWeb.Forms.ShareholderComponent do
  alias Brs.Helpers
  use BrsWeb, :live_component
  alias Brs.Forms.ShareHolder
  import Phoenix.HTML.Form

  @impl true
  def mount(socket) do
    changeset = ShareHolder.changeset(%ShareHolder{})
  ...
  def update(%{owner_index: owner_index} = assigns, socket) when not is_nil(owner_index) do
    shareholder =
      Enum.at(assigns.form_data["shareholders"], String.to_integer(owner_index))
      IO.inspect(shareholder, label: "shareholder-----------------")

    changeset =
      %ShareHolder{}
      |> ShareHolder.changeset(shareholder)
              ...
              <li class="list-group-item table-responsive">
                ...
                <div class="mb-3">
                  <%= for {share, index} <- Enum.with_index(@form_data["share_information"] && @form_data["share_information"]["shares"] || []) do %>
                    <div class="flex flex-col bg-gray-100 p-2 border rounded">
  ...
  @impl true
  def handle_event("validate", %{"share_holder" => params}, socket) do
    updated_params = update_share_information(socket, params)

    changeset =
      socket.assigns.form.data
      |> ShareHolder.changeset(updated_params)
      |> Ecto.Changeset.apply_changes()
      |> Ecto.Changeset.change()
  ...
  def handle_event("save", %{"share_holder" => params}, socket) do
    updated_params = update_share_information(socket, params)
    IO.inspect(socket.assigns.form.data, label: "form data-----")
    IO.inspect(updated_params, label: "update params----")
    changeset = ShareHolder.changeset(socket.assigns.form.data, updated_params)

    if changeset.valid? do
      send_update(BrsWeb.Forms.OwnershipDetailsComponent,
        id: "form",
        new_shareholder: updated_params,
  ...
  def handle_event("issue-shares", %{"shares" => shares, "category" => category}, socket) do
    with {:ok, shares_number} <- parse_shares(shares),
         {:ok, params} <- build_share_params(socket, category, shares_number) do
      form =
        Brs.Forms.ShareInformation.add_shares(socket.assigns.form.data, params)
        |> Ecto.Changeset.change()
        |> Ecto.Changeset.apply_changes()
        |> Ecto.Changeset.change()
        |> to_form()
  ...
  def handle_event(
    ...
    with {:ok, direct_shares} <- parse_shares(direct_share_ownership) do
      ...
      updated_params =
        update_share_information(socket, %{direct_shareownership: trunc(indirect_ownership)})

      form = update_form(socket, updated_params)
      {:noreply, assign(socket, form: form)}
    else
      {:error, msg} -> {:noreply, put_flash(socket, :error, msg)}
    end
  ...
  def handle_event("cancel-shares", %{"category" => category}, socket) do
    form_data =
      remove_category_shares(socket, category)
      |> Helpers.struct_to_map()
      |> Helpers.stringify_keys()

    form = update_form(socket, form_data)
    {:noreply, assign(socket, form: form)}
  end
  ...
  defp update_share_information(socket, params) do
    current_share_info =
      ...
      |> case do
        ...
        share_info ->
          share_info |> Helpers.struct_to_map()
      end
      |> Helpers.stringify_keys()

    updated_share_info = Map.merge(current_share_info, params["share_information"] || %{})
    Map.put(params, "share_information", updated_share_info)
  end

  defp build_share_params(socket, category, shares_number) do
    share_info =
    ...
    category_shares =
      Enum.find_value(
        share_info["shares"],
        0,
        &((&1["category"] == category && &1["number"]) || 0)
      )

    with {:ok, percentage} <- calculate_percentage(total_shares, shares_number),
      ...
      _ -> {:error, "Invalid share data"}
    end
  end

  defp remove_category_shares(socket, category) do
    ...
    case socket.assigns.form.data do
      %{share_information: %{shares: shares} = share_info} = form_data ->
        ...
        with {:ok, percentage} <- calculate_percentage(total_shares, shares_number) do
          %{
            form_data
            | share_information: %{share_info | shares: updated_shares, percentage: percentage}
          }
        end
  ...
  defp parse_shares(shares) do
    case Integer.parse(shares) do
      ...
      _ -> {:error, "Invalid number of shares. Please enter a positive integer."}
    end
  end

  defp calculate_percentage(total_shares, shares) do
    if shares > 0 do
      {:ok, Float.round(shares / total_shares * 100, 2)}
    else
      {:ok, 0}
    end
  end
